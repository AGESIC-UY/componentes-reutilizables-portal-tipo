<div class="Box Box--mf Box--listadoHorizontal">
		<div class="Box-header">
				<h3 class="Box-name">Noticias</h3>
		</div>
		<div class="Box-body">
				<ul class="Box-gridList Grid Grid--rowSize3">
						<li class="Grid-item u-md-size1of3">
								<a href="#">
										<div class="Image-cover">
											<img class="Box-image" src="./img/ruta.jpg" alt="">
										</div>
										<span class="Box-info Box-info--absolute">DD/MM/AAAA</span>
										<span class="Box-title">Lorem ipsum dolor consectetur adipiscing nulla architecto quat molestias aliquam</span>
								</a>
						</li>
						<li class="Grid-item u-md-size1of3">
								<a href="#">
										<div class="Image-cover">
											<img class="Box-image" src="./img/hole.jpg" alt="">
										</div>
										<span class="Box-info Box-info--absolute">DD/MM/AAAA</span>
										<span class="Box-title">Lorem ipsum dolor consectetur adipiscing nulla architecto quat molestias aliquam</span>
								</a>
						</li>
						<li class="Grid-item u-md-size1of3">
								<a href="#">
										<div class="Image-cover">
											<img class="Box-image" src="./img/uruguay.jpg" alt="">
										</div>
										<span class="Box-info Box-info--absolute">DD/MM/AAAA</span>
										<span class="Box-title">Lorem ipsum dolor consectetur adipiscing nulla architecto quat molestias aliquam</span>
								</a>
						</li>
				</ul>
		</div>
		<div class="Box-footer">
				<a href="#" class="Box-more">Acceder a más noticias</a>
		</div>
</div>

<!--
 * Clases modificadoras
	 - De color: Box--color1, Box--color2, Box--color3, Box--color4, Box--color5, Box--color6, Box--color7, Box--color8

 * Textos
	 - Box-name: Automático.
	 - Box-info: Fecha automática.
	 - Box-title: Automático, hasta 90 caracteres.
	 - Box-more: Automático.
-->