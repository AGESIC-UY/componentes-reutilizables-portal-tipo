<div class="Box Box--mf Box--microaccesos">
	<div class="Box-header">
		<h3 class="Box-name">Lista de microaccesos</h3>
	</div>
	<div class="Box-body">
		<div class="Microaccesos-item">
		 <a href="#">	
				<div class="Flag">
					<div class="Flag-image">
							<img src="./img/60x60.png" alt="">
					</div>
					<div class="Flag-body">
							<span class="Box-title">Microacceso destacado</span>
					</div>
				</div>
			</a>
		</div>
		<div class="Microaccesos-item">
		 <a href="#">	
				<div class="Flag">
					<div class="Flag-image">
							<img src="./img/60x60.png" alt="">
					</div>
					<div class="Flag-body">
							<span class="Box-title">Microacceso 2</span>
					</div>
				</div>
			</a>
		</div>
		<div class="Microaccesos-item">
		 <a href="#">	
				<div class="Flag">
					<div class="Flag-image">
							<img src="./img/60x60.png" alt="">
					</div>
					<div class="Flag-body">
							<span class="Box-title">Acceso 3</span>
					</div>
				</div>
			</a>
		</div>
		<div class="Microaccesos-item">
		 <a href="#">	
				<div class="Flag">
					<div class="Flag-image">
							<img src="./img/60x60.png" alt="">
					</div>
					<div class="Flag-body">
							<span class="Box-title">Microacceso número 4</span>
					</div>
				</div>
			</a>
		</div>
		<div class="Microaccesos-item">
			<a href="#">	
				<div class="Flag">
					<div class="Flag-image">
							<img src="./img/60x60.png" alt="">
					</div>
					<div class="Flag-body">
							<span class="Box-title">Microacceso 5</span>
					</div>
				</div>
			</a>
		</div>
		<div class="Microaccesos-item">
			<a href="#">	
				<div class="Flag">
					<div class="Flag-image">
							<img src="./img/60x60.png" alt="">
					</div>
					<div class="Flag-body">
							<span class="Box-title">Microacceso 6</span>
					</div>
				</div>
			</a>
		</div>
		<div class="Microaccesos-item">
			<a href="#">	
				<div class="Flag">
					<div class="Flag-image">
							<img src="./img/60x60.png" alt="">
					</div>
					<div class="Flag-body">
							<span class="Box-title">Microacceso 7</span>
					</div>
				</div>
			</a>
		</div>
		<div class="Microaccesos-item">
			<a href="#">	
				<div class="Flag">
					<div class="Flag-image">
							<img src="./img/60x60.png" alt="">
					</div>
					<div class="Flag-body">
							<span class="Box-title">Microacceso 8</span>
					</div>
				</div>
			</a>
		</div>
	</div>
</div>

<!--
 * Clases modificadoras
	 - De diseño: Box--microaccesos-fewItems (cuando son menos de 4 items), Box--microaccesos-noTitle (se elimina el título)

 * Imagen
	 - Debe ser formato 1:1

 * Textos (No dejar etiquetas html vacías cuando los campos opcionales quedan sin valor, se eliminan)
	 - Box-name: Óptimo 50 caracteres.
	 - Box-title: Óptimo 40 caracteres. Obligatorio.
 
 * Comentarios:
	 - Está pensado para un mínimo de 2 y máximo de 8 microaccesos 
-->