class texto_a_voz extends HTMLElement {
	...
	connectedCallback() {
		...
		// Crear combo box para seleccionar voz
		const selectVoz = document.createElement("select");
		selectVoz.setAttribute("class", "Form-widget Form-select");
		selectVoz.setAttribute("data-wcfid", "select_voz");
		selectVoz.setAttribute("aria-label", "Elegir voz para la lectura");
		selectVoz.style.marginLeft = "0.5em";
		selectVoz.style.display = "inline-block";
		selectVoz.style.verticalAlign = "middle";
		
		// Opción por defecto
		const optionDefault = document.createElement("option");
		optionDefault.value = "";
		optionDefault.textContent = "Elegir voz";
		optionDefault.disabled = true;
		optionDefault.selected = true;
		selectVoz.appendChild(optionDefault);
		
		// Función para cargar voces disponibles
		const cargarVoces = () => {
			const voces = window.speechSynthesis.getVoices();
			// Filtrar voces en español o mantener todas si no hay filtro
			const vocesFiltradas = voces.filter(voz => voz.lang.includes('es') || voz.lang.includes('ES'));
			const vocesDisponibles = vocesFiltradas.length > 0 ? vocesFiltradas : voces;
			
			// Limpiar opciones existentes (excepto la opción por defecto)
			while (selectVoz.children.length > 1) {
				selectVoz.removeChild(selectVoz.lastChild);
			}
			...
			vocesDisponibles.forEach((voz, index) => {
				...
				option.value = index;
				// Mostrar nombre de la voz y idioma
				const nombreVoz = voz.name || `Voz ${index + 1}`;
				const idiomaVoz = voz.lang ? ` (${voz.lang})` : '';
				option.textContent = nombreVoz + idiomaVoz;
				option.setAttribute("data-voice-name", voz.name);
				option.setAttribute("data-voice-lang", voz.lang);
		...
		// Almacenar la voz seleccionada en una variable accesible globalmente
		window.vozSeleccionadaTTS = null;
		...
		selectVoz.addEventListener('change', function() {
			...
			if (!isNaN(selectedIndex) && selectedIndex >= 0 && selectedIndex < vocesDisponibles.length) {
				window.vozSeleccionadaTTS = vocesDisponibles[selectedIndex];
			} else {
				window.vozSeleccionadaTTS = null;
			}
		});
		...
		div.appendChild(selectVoz);
		wcf_allInstances.allCompsIds[tagId]["select_voz"] = selectVoz;
...
// Helper functions for button icons (defined outside class so they're accessible from static callback)
const createPlayIconButton = () => {
	...
	path.setAttribute("d", "M8 0L16 8L8 16L8 0Z");
	svg.appendChild(path);
	return svg;
};

const createPauseIconButton = () => {
	const svg = document.createElementNS("http://www.w3.org/2000/svg", "svg");
	svg.setAttribute("width", "16");
	svg.setAttribute("height", "16");
	svg.setAttribute("viewBox", "0 0 16 16");
...
const createRestartIconButton = () => {
	...
	svg.style.display = "inline-block";
	const path = document.createElementNS("http://www.w3.org/2000/svg", "path");
	// Triángulo apuntando hacia la izquierda (espejo del play)
	path.setAttribute("d", "M8 0L0 8L8 16L8 0Z");
	svg.appendChild(path);
	return svg;
};

const updateButtonContentWithIcon = (btn, text) => {
	btn.innerHTML = "";
	// Determine which icon to use based on text
	if (text === "Pausar" || text.toLowerCase().includes("paus")) {
		btn.appendChild(createPauseIconButton());
	} else if (text === "Reiniciar" || text.toLowerCase().includes("reinici")) {
		// Use restart icon (left-pointing triangle) for "Reiniciar"
		btn.appendChild(createRestartIconButton());
	} else {
		// Use play icon for "Leer", "Reanudar", or any other text
		btn.appendChild(createPlayIconButton());
	}
...
class boton_leer extends HTMLElement {
	static attributes = {
		button: {
			data: {},
			callback: function(instance, name, data, oldValue, newValue) {
				// This callback handles button content updates with icons
				const tagId = instance.getAttribute("id");
				if (tagId && newValue !== null && newValue !== undefined) {
					// Try to get the button element by wcfid
					const buttonElem = wcf_allInstances.getInstanceElemByWcfid(tagId, "button");
					if (buttonElem) {
						updateButtonContentWithIcon(buttonElem, newValue || "Escuchar contenido");
					}
				}
			}
		}
	...
	connectedCallback() {
		...
		/* Function to update button content with icon and text - uses global functions */
		const updateButtonContent = (btn, text) => {
			updateButtonContentWithIcon(btn, text);
		};

		/* get the parameter if exists */
		var buttontext = this.getAttribute("button");
		if (!buttontext) {
			buttontext = "Escuchar contenido";
		}
		updateButtonContent(button, buttontext);

		/* Store update function reference in button for later use */
		button._updateContent = updateButtonContent;
		button._buttonElement = button;
		this._updateButtonContent = updateButtonContent;
		this._buttonElement = button;

		/* Register the attribute for the observer - NOT as wcfAttr so we handle updates ourselves */
		// Use the callback from static attributes which handles icon updates
		const buttonCallback = boton_leer.attributes.button.callback;
		wcf_allInstances.attrObserveRegister(tagId,"button", {"elem": button, "type":"button", "wcfAttr": false}, buttonCallback)
...
customElements.define("comp-boton-leer", boton_leer);
	...
	class Lector{
		...
		leer() {
			...
			if (window.vozSeleccionadaTTS) {
				utterance.voice = window.vozSeleccionadaTTS;
				utterance.lang = window.vozSeleccionadaTTS.lang;
			}
